...
class Sql:
    summary_sql = "select a.part_no,e.name as di_standard,a.size as size, a.consignment_amount as consignment_amount, coalesce(b.sum,0) as usage_sum,coalesce(c.sum,0) as delivery_sum,coalesce(d.sum,0) as return_sum from website_otitem a" \
            ...
            " order by a.part_no;"
    monthly_usage_sql = "select a.part_no, e.name as di_standard,a.size as size, coalesce(b.sum,0) as current_month_sum,coalesce(c.sum,0) as second_month_sum,coalesce(d.sum,0) as third_month_sum from website_otitem a" \
                        ...
                        " left join website_otdistandard e on (a.di_standard_id = e.id)" ;
    monthly_return_sql = "select a.part_no, e.name as di_standard,a.size as size, coalesce(b.sum,0) as current_month_sum,coalesce(c.sum,0) as second_month_sum,coalesce(d.sum,0) as third_month_sum from website_otitem a" \
    ...
    monthly_delivery_sql = "select a.part_no, e.name as di_standard,a.size as size, coalesce(b.sum,0) as current_month_sum,coalesce(c.sum,0) as second_month_sum,coalesce(d.sum,0) as third_month_sum from website_otitem a" \
